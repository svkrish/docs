...
## Table of Contents
  ...
  * [Our custom Pipelines](#our-custom-pipelines)
...
pipelines, and the goal of the [devops-pipelines](https://github.com/ibm-garage-ref-storefront/devops-pipelines) repository is to help you get going.
...
This repository includes two [directories](https://github.com/ibm-garage-ref-storefront/devops-pipelines/tree/master/pipelines), `experimental`(pipelines that are not production-ready and are considered, proof of concept), and `stable`(pipelines that are production ready). It also includes the following pipelines:
...
2. Now drag and drop your pipelines and tasks to any of these folders or the one you created.

3. You must update the `configmap` and `secret` we provided for you. But first, create another repository such as [pipelines-server](https://github.com/ibm-garage-ref-storefront/pipelines-server). In this repo `pipelines-server` you will be hosting your pipelines as Git releases. Do not forget to create a README.md file.
...
- **GitOps**: This task will update the the deployment manifests residing in [gitops-dev](https://github.com/ibm-garage-ref-storefront/gitops-dev) repository.
